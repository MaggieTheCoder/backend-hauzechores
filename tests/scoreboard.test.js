    ...
    describe("/scoreboard", ()=> {
        describe("POST", ()=> {
            it("creates a new score to the database", async () => {
                ...
                const[[scoreEntries]] = await db.query(`SELECT * FROM Scoreboard WHERE houseID = "test house"`);
...
it("returns all scores from the database" , async ()=> { 
    let scores;
    await Promise.all([
        db.query("INSERT INTO Scoreboard (score, userID, houseID) VALUES (?, ? , ?)",  [14, 5, "test house1"]),
        db.query("INSERT INTO Scoreboard (score, userID, houseID) VALUES (?, ? , ?)",  [15, 2, "test house2"]),    
    ]);
    [scores] = await db.query("SELECT * FROM Scoreboard");

    const res = await request(app).get('/scoreboard').send();
    expect(res.status).to.equal(200);
    expect(res.body.lenght).to.equal(2);

    res.body.beforeEach((scoreRecord)=> {
